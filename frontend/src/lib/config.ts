...
//      static/config.js ships "/api/v1": the read API serves this page, so
//      the API is same-origin — never the in-cluster Service DNS name,
...
//      via Vite's dev-server proxy, and for the built site the read
//      API serves).
//
