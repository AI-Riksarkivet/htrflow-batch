...
while no release exists yet.) That repo's CI is GitHub Actions; add
`--ci azure` for Azure Pipelines instead. Either way `init` writes the
Argo CD hook, `argocd/apply.yaml`.
...
.github/workflows/render.yml  # the CI: validate, policy-check, render (--ci github, the default)
azure-pipelines.yml        # the same CI on Azure Pipelines, in its place (--ci azure),
.azure-pipelines/          #   with the steps template its stages share
argocd/apply.yaml          # the Argo CD PostSync hook that applies it: set REPO_URL, REPO_BRANCH, HTRFLOW_APPLIED_BY
```
...
1. Add (or reuse) a pipeline in `pipelines/<id>.yaml` — a digest-pinned
   ...
   **permanent name for a recipe**: once any campaign has run against it,
   ...
   (`demo-v2.yaml`) instead of editing this one. `validate` and `render`
   refuse an edit while a rendered campaign still names the pipeline, and
   `apply` refuses one while a campaign in the cluster still runs it; after
   that it is a review convention, so protect `main` and require review on
   this repo.
...
3. Open a PR. CI runs `htrflow-campaigns validate` on it, then the
   **Policy** job: the same Kyverno `ClusterPolicy` objects the cluster
   ...
   same sentence. Keep the job's `POLICY_*` variables in step with the
   release's `security.*` values.
4. Merge. CI on `main` runs `htrflow-campaigns render` and commits the
   result under `rendered/` (see `.github/workflows/render.yml`, or
   `azure-pipelines.yml`).
...
future work — until it exists, results are kept forever.
