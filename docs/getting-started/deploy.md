...
| `network.iiifCidrs` | The address ranges of your IIIF source, and of any host that `images:` volumes point at. Campaign pods may reach these on ports 80 and 443. Any range is accepted, a catch-all covering every IPv4 address included, and each loses the cluster, node, API server, link-local, loopback and `network.privateCidrs` ranges that lie inside it. The chart default names one specific IIIF host; `values-prod.yaml` empties it and the render refuses an empty list, so always set this. |
| `network.s3Cidrs` | The S3 endpoint's address ranges. Campaign pods and the web front's read API both reach S3 through this, on `network.s3Ports` (default 443) and no other port. Set that too if your endpoint answers elsewhere. `values-prod.yaml` sets `network.s3InNamespace: false` (no route to the dev stack's in-namespace RustFS), and then an empty list is refused: a campaign pod with no route to S3 would fail every volume after its GPU time. |
| `network.clusterCidrs` | Your cluster's pod and service CIDRs. Warm-up pods get public egress except to these ranges and the node addresses. The chart default is one distribution's defaults; `values-prod.yaml` empties it and the render refuses an empty list, so set yours. |
| `network.privateCidrs` | The private ranges no wide egress range may reach, on top of the cluster and node ranges and the link-local and loopback blocks, which are always excluded. The default is the three private blocks and carrier-grade NAT space (`100.64.0.0/10`); set it only if your network is planned differently. A range you name in `iiifCidrs` or `s3Cidrs` that lies inside one of them stays reachable. |
