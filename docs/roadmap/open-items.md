...
| D14 | Pod security + egress NetworkPolicy | **built** — restricted pod specs, namespace default-deny + per-role allowlists, read-only offline model cache with a converter-rendered warm-up Job, digest-gated control-plane images, and the chart's Kyverno ClusterPolicies (`security.policies.enabled`: digest pins, the image allow-list, model revisions — the *only* enforcement of the last two since Task 22), plus optional cosign `verifyImages` ([Security](../development/security.md)); a sandboxed runtime is the open follow-up |
...
| — | Campaigns as Indexed Jobs + read-only status API | **built and running** on the k3s PoC (release `htr`, ns `htr-batch`) — no CronJob, no controller; converter renders manifests and applies them through the Kubernetes client, Kueue/Kubernetes own retries, one `htrflow-web` process serves the read API, the campaign browser and the viewer, live run log unchanged ([Campaigns (Indexed Jobs)](../how-it-works/campaigns.md), [Local k3s development](../development/local-k3s.md)) |
...
| — | Kyverno `ClusterPolicy` → `ValidatingPolicy` | open — Kyverno 1.19 deprecates the kind the chart ships and warns on every apply; the CEL-based `policies.kyverno.io/ValidatingPolicy` migration is a task of its own ([Evolution](evolution.md#other-items)) |
