...
| …that a browser can open: no backslash or control character, a port from 0 to 65535, and a host that is a name (letters, digits, `_` and `-` between the dots; an `xn--` label only when it spells left-to-right letters), a dotted IPv4 address, or a bracketed IPv6 address without a zone | Validation error naming the volume and what the browser would refuse. The viewer and the status page build every link with the browser's own URL parser, which throws on URLs Python's accepts |
...
| A volume id appears in at most one campaign per pipeline | Validation error naming both campaign files and the pipeline. Results are keyed `<pipeline>/<volume>/`, not by campaign, so two campaigns on one pipeline would run the same volume at once and write over each other's pages. Remove the volume from one of them — or retire the finished campaign first, by deleting its file. The same volume on two different pipelines is two sets of results, and allowed |
...
| **A change of `window:` — the campaign's, or `converter.yaml`'s cap — that changes how many pods a rendered, unpaused campaign runs at once is rejected** | `validate` and `render` print `campaign <name> runs <n> pods at a time and would now run <m>` and exit non-zero. Kueue compares a running Job's pod count, the smaller of its parallelism and its volume count, with the Workload it admitted, and when they differ it stops every running pod and queues the campaign again. A paused campaign runs no pods and its Workload holds no quota, which Kueue updates in place: pause the campaign, change the window once the pause is applied, then resume it. A change that leaves the count where it was (a window above the volume count either way) passes |
