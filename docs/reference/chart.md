...
| `network.iiifCidrs` | `["192.121.221.27/32"]` | What campaign pods may reach besides DNS and S3, on 443/80: your IIIF origin(s). An empty list is refused (it would fetch nothing); `values-prod.yaml` empties it, so a production install must name its own. The default is one IIIF origin's address — set this for your source. Volumes declared with `images:` hosted elsewhere need that host here too; `0.0.0.0/0` allows any origin. Every range here, in `network.s3Cidrs` and in `apply.gitCidrs` loses the internal ranges inside it — cluster, node, API server, link-local, loopback and `network.privateCidrs` ([Security](../how-it-works/security.md)) |
