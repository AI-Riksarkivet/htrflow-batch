...
| `queue.flavors` | `[]` | More than one sort of GPU: one ResourceFlavor per entry (`name`, `nodeLabels`, optional `nodeTaints`, and a `quota` of `cpu`, `memory` and `nvidia.com/gpu`, each above 0), in one resource group Kueue tries in this order. A flavor tolerates its own taints. Set, it replaces `queue.flavor` and `queue.resources`; with `createFlavor: false` the flavors are the cluster's, referenced by name. `converter.yaml`'s `flavors` must repeat the names and node labels ([Several sorts of GPU](../how-it-works/queueing.md#several-sorts-of-gpu)) |
