...
| `PAGE_TIMEOUT_SECONDS` | `600` | Wall-clock budget of one page inside htrflow. Past it the page fails, like one whose worker thread died, and the pipeline is rebuilt. Threads a released pipeline cannot stop (a worker stuck in its model, the helper of a page past its budget) are counted; at 8 the run ends transient so the retry gets a fresh pod |
