...
**E. Pipeline chip regression (lost in Task 7 — restore, no-functionality-loss rule).** Before Task 7 the campaign card's pipeline chip was a button whose `title` listed the step names joined by ` → ` and whose click toggled an inline `<pre>` with the pipeline YAML (see `git show 166d1d5:frontend/src/lib/components/CampaignCard.svelte` lines 19-80 and `status.ts` fields `pipeline_steps`/`pipeline_yaml`). Restore it: `GET /api/v1/jobs/{ns}/{name}` (JobDetail) gains `pipelineSteps: string[]` (each step's `step` value, in order) and `pipelineYaml: string`, read from the `htr-pipeline-<id>` ConfigMap's `pipeline.yaml` key (the reader already fetches the campaign ConfigMap; add one `get_configmap` call; missing ConfigMap → empty list / empty string, never an error). Not on JobSummary (size). `frontend/src/lib/api.ts` schema + `CampaignCard.svelte`: chip is a `<button>` again with the `title` and the YAML toggle, exactly the old markup/behaviour; static chip only while the detail has not loaded. Tests: projection (steps parsed from a ConfigMap fixture, missing ConfigMap tolerated), Zod, component (title text, click reveals YAML). Docs: `docs/reference/frontend.md` derivation rules, `docs/how-it-works/live-run-log.md` if it lists JobDetail fields. Frontend budget ≤ 2500, web/api budget honoured.

**F. Campaign card: collapsed by default, source link restored, log + viewer reachable without unfolding (Morgan, 2026-09-02).**
- *Source slot (regression from Task 7).* The old card had three fixed link slots per volume — open · source · log (`git show 166d1d5:frontend/src/lib/components/CampaignCard.svelte` ~lines 180-225); Task 7 dropped `source`. Restore: `VolumeView` gains `sourceUrl: string | null` — the URL half of the volume's `volumes.txt` line when it is a manifest URL, `null` for an `images:` line (the API reads the ConfigMap already; `line.split("\t", 1)[1]`). The card renders the three slots again with the old gap-not-shift markup; "open" links `iiifUrl` for `done` volumes and, as before, the source manifest in UV (`uv.html#?manifest=<sourceUrl>`) for every other state that has one.
- *Collapsed by default.* `collapsed = $state(true)`; remember the per-campaign choice in `localStorage` (wrapped in try/catch; key `htrflow.card.<ns>/<name>`); the disclosure button and `aria-expanded` unchanged.
- *Reachable while collapsed.* The collapsed header row shows, besides phase and counts, a compact "latest" strip: the most recently touched volume (newest `active`, else newest `done`) with its open · source · log links, so UV and the run log are one click away without unfolding; the failures block stays visible while collapsed (it already is).
- Tests: Zod + projection for `sourceUrl` (manifest line → URL, images line → null), component tests: collapsed by default, the strip's three links, localStorage round-trip, expanded table shows all three slots. Docs: `docs/reference/frontend.md` derivation rules. Frontend budget ≤ 2500 (if F pushes it over, raise to 2700 and say so — this is restored functionality, not growth).

**G. Human-friendly error messages on every surface (Morgan, 2026-09-02: "a lot more human friendly").** Rule for every message a person reads: say *what happened*, *where* (file / volume / stage), and *what to do next*, in plain sentences — no JSON blobs, no Python reprs, no internal names (`ApiUnreachable`, `stage: stream`, exit codes, `Field required`, `loc` paths like `volumes.0.id`). Machine-readable forms stay where machines read them (the wrapper's termination-log JSON, exit codes, the API's structured fields); the humanising happens at the edge that talks to people.
- *Read API → frontend.* `VolumeView.reason` becomes structured: `{stage, permanent, error}` parsed from the wrapper's termination JSON (fallback `{stage: null, permanent: null, error: <raw text>}`), plus `state: "failed"` keeps working. The card and the failures block render one sentence per known error, e.g. `MAX_SECONDS` → "Stopped after the time budget ran out while processing pages; the next attempt resumes from the pages already done." · `SIGTERM` → "The pod was stopped by the cluster (node drain or pause); it will be retried." · manifest errors (stage setup, permanent) → "The IIIF manifest could not be read: <error>. Fix the manifest URL in the campaign file — this volume will not be retried." · `verify failed: missing=[…] failed=[…]` → "3 pages could not be processed (p012, p045, p101); the volume is retried automatically." · unknown → "Failed while <stage in words>: <error>". Stage names map to words (setup → "reading the manifest", resume → "checking earlier results", load → "loading the model", stream → "processing pages", verify → "checking results", publish → "publishing results"). Page banners: "API unreachable: HTTP 503 — showing the last good list" → "Can't reach the campaign service right now (HTTP 503). Showing the last list we received; retrying every minute." · "invalid API response" → "The campaign service answered in a form this page doesn't understand. Reload the page; if it keeps happening, the page and the service are different versions." · 404 detail "job not found" → "This campaign no longer exists (finished campaigns are removed after 24 hours)."
- *Converter CLI (campaign authors).* Every problem is `path/to/file.yaml: <plain sentence> — <what to do>`, e.g. `campaigns/broken.yaml: volume 1 ("a/b") has an id with characters that are not allowed — use only letters, digits, ".", "_" and "-", at most 63 characters` · `pipelines/demo-v1.yaml: "image" is missing — add image: <registry>/<repo>@sha256:<64 hex digits>` · `campaigns/kyrk.yaml: volume "R1" is listed twice — remove the duplicate` · `campaigns/kyrk.yaml: window must be a whole number of 1 or more (got "5" — quotes make it text)` · a closing line `2 problems in 1 file — nothing was rendered`. `_problems()` translates pydantic's `missing`/`int_type`/`greater_than_equal` error types and `loc` into these sentences (one small mapping table); validators keep raising the sentence directly. Tests: `test_parse.py`'s substring assertions are updated deliberately to the new wording (this is a feature change, not a refactor — say so in the commit); every fixture under `tests/fixtures/bad` gets its expected sentence asserted verbatim in one table-driven test so wording is pinned.
- *Wrapper run log.* Terminal lines stay machine-stable for the frontend's live-log stop rule (`COMPLETE`, `permanent failure in`), but each gets a trailing plain-language sentence, e.g. `permanent failure in setup: manifest is not JSON — the manifest URL returned HTML; check the URL in the campaign file`. Exit codes and the termination JSON are unchanged.
- Docs: `docs/how-it-works/failure-handling.md` gets the message table (error → sentence → what the operator does). Tests for every mapping; no untranslated raw path reaches the UI (component test with a raw-JSON reason asserts the sentence, not the JSON).
...
- [ ] Step 1: failing tests for A, B, C. - [ ] Step 2: implement A–G (G last; it changes test wording deliberately). - [ ] Step 3: `make ci`, `bun run test|check|build`, budgets, zensical. - [ ] Step 4: Commits `fix(ci): dagger test source is the repo minus an exclude list (B63)`, `feat(api): PartiallyFailed phase for campaigns with some failed indexes (B63)`, `fix(frontend): loaded volume pages survive a poll (B63)`, `fix(make): install-devstack refuses to drop the GPU device plugin while GPU pods exist (B63)`, `fix(frontend): pipeline chip shows steps and YAML again (B63)` (+ its `feat(api)` commit), `feat(frontend): cards collapsed by default with a latest-volume strip; source link restored (B63)` (+ `feat(api): VolumeView.sourceUrl (B63)`), `feat: human-friendly error messages — API reasons, page banners, converter problems, run-log lines (B63)` as several small commits per surface.

### Task 21: `apply` talks to the API server, not to `kubectl`

**Why:** Morgan: "I also don't like the subprocess part too much." Task 14's `apply` shells out to `kubectl`; tests assert argv strings; prune semantics are kubectl's. Replace with the official `kubernetes` client (already used by `packages/api`/`web` — one client library and one auth path in the repo), server-side apply, our own selector prune, and tests against a fake API. Command, flags (minus `--kubectl`), docs and behaviour stay.

**Files:** `packages/converter/pyproject.toml` (+ `kubernetes`), `packages/converter/src/htrflow_converter/cli.py` (`apply`) → new `packages/converter/src/htrflow_converter/cluster.py` (≤ 150 lines): `Cluster(namespace)` loading kubeconfig or in-cluster config; `apply(obj)` = server-side apply via `PATCH` with `content_type="application/apply-patch+yaml"`, `fieldManager="htrflow-campaigns"`, `force=True` (use `CustomObjectsApi`/`DynamicClient` or the typed `BatchV1Api.patch_namespaced_job` + `CoreV1Api.patch_namespaced_config_map` with `_content_type` — pick the one that works against the PoC, record it); `prune(rendered_names)` = list Jobs + ConfigMaps by `CAMPAIGN_SELECTOR`, delete those not rendered (Jobs with `propagation_policy="Background"`), print `pruned: <kind>/<name>`; `sync_pause(job_name, suspended, wait)` = list Workloads (`kueue.x-k8s.io/v1beta1`) by `kueue.x-k8s.io/job-uid=<uid>` (uid from the applied Job's response), patch `spec.active` only when it differs, bounded wait for a paused campaign's Workload, non-zero when absent. `--dry-run` = render + print the objects that would be applied/pruned, no API calls. Delete the `kubectl` stub tests; `test_apply.py` uses a fake client (a small class recording calls / answering lists) — assert pipelines applied before campaigns, prune deletes only unrendered selector-matched objects, pause patch only on difference, wait-then-fail. Docs: `campaign-yaml.md#pausing`, `campaigns.md`, `examples/campaigns/README.md`, the Argo PostSync hook (ServiceAccount with RBAC for jobs/configmaps/workloads — add `charts/htrflow-batch/templates/apply-rbac.yaml` behind `apply.rbac.enabled: false`), `render.yml` comment. E2E on the PoC: `make campaigns-apply` with a paused + running campaign and `--prune` as in Task 14; record. Budget converter ≤ 1000 (expect ≈ same). Tiny commits.

### Task 22: Policy lives in Kyverno, not in the converter (Morgan: "policy should be part of kyverno")

**Why:** ~150 lines of the converter are cluster policy — allowed image registries, digest pinning, pinned model revisions — enforced at render time only. Kyverno enforces the same rules on every Pod and ConfigMap the cluster admits, whoever rendered them; the campaigns repo's PR check runs the same policies with the Kyverno CLI against the rendered output, so authors still fail early with Kyverno's message.

**Files:**
- `charts/htrflow-batch/templates/policies/` (rename/move the existing `kyverno.yaml` verify-images policy here; behind `policies.enabled`, default `false` until the target cluster has Kyverno — ai-dev's I04): `ClusterPolicy htrflow-batch-images-pinned-<ns>` (every container/initContainer image in Jobs of the namespace matches `@sha256:[0-9a-f]{64}`), `htrflow-batch-images-allowed-<ns>` (image registry/repo prefix in `policies.allowedImageRepos`, values list), `htrflow-batch-model-revision-<ns>` (ConfigMaps labelled `CAMPAIGN_SELECTOR` with a `pipeline.yaml` key whose `model_settings:` blocks carry a 40-hex `revision:` — Kyverno `validate` with a regex over the data string; if this is not expressible cleanly in Kyverno, keep the revision rule in the converter and say so). All `validationFailureAction: Enforce`, `background: true`, messages human-readable ("image must be pinned by digest: <image>").
- `packages/converter`: delete `allowed_image_repos`, `require_model_revision`, `_IMAGE_RE` digest enforcement (keep a *shape* check that `image` is non-empty), `_check_step_revisions`, their tests and fixtures; `converter.yaml` docs updated (fields removed → a validation error if still present, with a message pointing at the chart's `policies.*`). Budget drops (report the number).
- Campaigns-repo CI: `examples/campaigns/.github/workflows/render.yml` gains a step `kyverno apply <policies dir> --resource rendered/` using the Kyverno CLI (pinned version), policies fetched from the htrflow-batch release (`helm template … --show-only templates/policies/*` or a published `policies/` tarball — pick the simpler, document it); `htrflow-campaigns validate` docs say what it no longer checks.
- PoC: add Kyverno to `charts/htrflow-devstack` as an optional dependency (`kyverno.enabled`, pinned chart version) so `make install-devstack` installs it; enable `policies.enabled` on the PoC; E2E: a campaign with an unpinned image and one with a foreign registry are REJECTED at apply with the policy message (record the exact message); a compliant campaign runs; Kyverno CLI in CI reproduces the same rejections on the rendered output.
...
### Task 24: Wrapper simplifications (2026-09-02)

**Brief:** `docs/superpowers/audits/2026-09-02-wrapper-simplifications.md` — companion to the audit, line numbers against `a05e89b`. Items 1–5 required (one S3 put path in `ResultStore`; one `S3_PREFIX` join via `Config.root_key`; pydantic parses the env in `Config.from_env`, `_bool` deleted; one htrflow exception map via `driver.build_pipeline` shared with warm-up; one ALTO reader in `publish.alto_dims`, `viewer.parse_alto_dims` deleted with the three `test_viewer.py` call edits — the only test edit allowed); items 6–9 optional (`PageStream` partial, `PageStream.error` decision, drop the `fetched` alias or the class name — pick one, shared canvas walk in `iiif` last or skipped); items 10–11 are KEEP. No behaviour change: existing tests pass unmodified except where an item says otherwise; env contract, exit codes, S3 keys and log lines byte-identical. One item per commit; verification block after each commit; wrapper budget ≤ 2000 (expect a drop — report the number). Runs directly after Task 23's review.

### Task 25: Wrapper → Kubernetes layer (2026-09-02)

**Brief:** `docs/superpowers/audits/2026-09-02-wrapper-to-kubernetes.md`. Items 1–3 required: (1) `MAX_SECONDS` watchdog → pod `activeDeadlineSeconds` rendered per pipeline (`max_seconds`), env var gone, timer/`on_expiry`/`RunState.terminating` and their tests deleted, SIGTERM path is the budget path — **blocked on the cluster check first**: does a pod killed by `activeDeadlineSeconds` carry a `DisruptionTarget` condition on this k8s (v1.35)? If yes, the Job's `Ignore onPodConditions DisruptionTarget` rule would retry forever → narrow the rule or add an exit-143 rule; record the answer + k8s version in the commit body. The read API surfaces `status.reason: DeadlineExceeded` so the card can distinguish "budget exceeded" from "node drained" (Task 20 G phrases it). (2) writable-dir `mkdir -p` moves into the Job/warm-up shell prologue; `prepare_writable_dirs` deleted. (3) workdir `rmtree` on success deleted (emptyDir dies with the pod). Item 4: keep log shipping (decided). **Item 5: SKIP** — `_terminate`'s structured JSON stays; Task 20 G parses it. Goldens regenerated deliberately; converter budget net-negative or one-liners; wrapper shrinks (report). E2E on the PoC for item 1: a campaign whose pipeline has `max_seconds: 60` on a 60-page volume → the pod is killed by the deadline, termination log `SIGTERM`, API reason shows the deadline, index retried and resumed to completion (as the Task 8 evidence showed for the watchdog); record in the run log. Tiny commits; verification block after each.

### Task 26: ALTO viewer in the run viewer (Morgan, 2026-09-02)

**Why:** every page's ALTO is already public (`<volume_prefix>/alto/<page>.xml`, `application/xml`, same CORS-GET bucket as the run log), `manifest.json` carries `viewer_url`, and `iiif.json` links each canvas's ALTO via `seeAlso` — but the SPA offers no way to read or download a page's ALTO. Frontend-only.

**Files:** `frontend/src/lib/run.ts` (type `viewer_url` in `runManifestSchema`; derive `alto?: string` per page next to `source?`), `frontend/src/lib/components/PagesTable.svelte` (an `alto` column: "view" → `/alto?src=<url>`, "download" → fetch + `Blob` + object URL with the page's filename — `<a download>` is ignored cross-origin), new route `frontend/src/routes/alto/+page.svelte` (+ `alto.html` handling in the viewer's static serving/nginx like `/log`): query param `src` guarded by `isHttpUrl`, `raw` link, ThemeToggle, fetch the XML and render **text lines in reading order** (TextBlock/TextLine/String `CONTENT`, `WC` confidence as a tint like the log viewer's levels, namespace-agnostic parsing) with a **"raw XML" toggle** (pretty-printed); errors in the Task 20 G voice. Tests: `run.test.ts` (alto URL derivation), a component test for the column, a route test with a small ALTO fixture (text extraction incl. hyphenation `SUBS_CONTENT` if present, confidence tint buckets, raw toggle). Docs: `docs/reference/frontend.md`, `docs/getting-started/viewing.md`. Budget: frontend row → 2800 with the comment `# raised for the ALTO viewer route (Task 26)`; reuse the log route's styles rather than duplicating CSS. Tiny commits.

### Task 27: Configuration agreement — one test, one prefix, one reference page

**Why (found during Task 20, 2026-09-03):** three config idioms (wrapper: pydantic `Config` + hand-kept `_REQUIRED`/`_OPTIONAL` tables; web: a dataclass `from_env` plus two env reads outside it in `app.py`/`__main__.py`; converter: pydantic `ConverterConfig` from `converter.yaml`); seven chart↔converter literals that must agree by convention only (`namespace`, `queue`↔`queue.name`, `s3_secret`↔`s3.existingSecret`, `data_pvc`, `runtime_class`, plus the two policy switches until Task 22 removes them from the converter); the results base under four names (`publicResultsBase`, `PUBLIC_RESULTS_BASE`, `HTRFLOW_PUBLIC_RESULTS_BASE`, `public_results_base`); `docs/audits/2026-08-26-hardcoded-inventory.md` describes a repo that no longer exists.

**Deliverables (runs after Task 22, before Task 12):**
- `packages/converter/tests/test_chart_agreement.py`: loads `charts/htrflow-batch/values.yaml` and `ConverterConfig()` defaults and asserts every shared key agrees, the mapping written once as a table in the test (the drift test the examples' "keep in sync by hand" comment asks a human to do).
- `packages/web`: the two stray env reads move into `Config.from_env`; one env-prefix decision written down (wrapper keeps bare names as the in-pod contract the Job spec writes; web keeps `HTRFLOW_`), documented in `docs/reference/wrapper.md` and the web README.
- A generated `docs/reference/configuration.md` (a small script under `scripts/` that renders every key, its source, default and which surfaces must agree — from the three models and the chart values, so it cannot drift) replacing the stale inventory; the old audit stays as history with a one-line pointer.
- **Security additions (Morgan, 2026-09-03 — "improve how configs are handled, security and cleanness"):** (a) `charts/htrflow-devstack` refuses to render RustFS with an empty or default secret key unless `devStack.insecureDefaults: true` (values comment says why; the PoC values set it explicitly); (b) a wrapper test asserting no env name in `_REQUIRED`/`_OPTIONAL` (or their successor) matches `KEY|TOKEN|PASSWORD|SECRET_ACCESS` — secrets are mounted files, never env — plus that sentence in `config.py`'s docstring and `docs/reference/wrapper.md`; (c) the generated reference page carries a *Security* column per key: secret? (file / Secret name / none) and enforced by (cluster / render / nobody), with the unauthenticated read API and the public results bucket stated in one line each.
- **Unify the three config models on one idiom** (pydantic `BaseModel` with a `from_env`/`from_yaml` classmethod of the same shape, one documented source per package — env for wrapper and web, `converter.yaml` for the converter — replacing the hand-kept `_REQUIRED`/`_OPTIONAL` tables and the web dataclass); zero behaviour change, every existing config test unmodified.
- Budgets: converter ≤ 1000, web ≤ 600, wrapper ≤ 2000; tiny commits.
