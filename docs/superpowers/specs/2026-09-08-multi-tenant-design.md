...
| `NetworkPolicy` set (default-deny, DNS, batch, warm-up, apply) | tenant chart | five per namespace |
...
| campaign `ConfigMap` + Indexed `Job` (submitter-labelled on the partner path) | converter, applied from CI | one per ≤ 200 volumes |
...
- No CRD, no controller, no state files in the bucket — as before.
...
| R10 | A GitHub login can be renamed, and the S3 prefix and label are built from it. | Rename policy: leave old results where they are, or key on the numeric account id instead. | Morgan |
