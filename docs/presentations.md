...
|---|---|---|
| **1. Distributed htrflow** | From one machine to many nodes, a rough architecture, a pod per archival volume, a Job per campaign, the window and the queue, priority, the bucket, git as the interface, the status page and the viewer | [slides](slides/part-1-from-one-folder-to-the-archive.html) · [PDF](slides/part-1-from-one-folder-to-the-archive.pdf) |
