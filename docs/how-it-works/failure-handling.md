...
| `terminationGracePeriodSeconds` | `120` | The wrapper's SIGTERM handler joins the log-shipping thread (up to 30 s) and then ships the run log one last time through a bounded S3 client (5 s connect, 30 s read, 2 attempts) — ~100 s worst case on a slow endpoint. The default 30 s would SIGKILL the pod mid-cleanup, losing the complete run log and the clean exit 143 |
