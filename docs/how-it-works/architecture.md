...
The web front (`packages/web`) is a passive extra piece. It is a projection
of live Job, Pod and ConfigMap state, plus each running volume's
`progress.json`. It also serves the status page and the Universal Viewer. The
one thing it writes is each campaign's status ConfigMap, the record that keeps
a campaign on the page after the Job's TTL has deleted the Job
...
| **apply**: `cluster.py` | Server-side apply under field manager `htrflow-campaigns`, the label-selected prune, the pause patch on `workload.spec.active`, and the Lease `htrflow-campaigns-apply` that lets one apply run at a time per namespace | `rendered/`, plus a kubeconfig or the hook pod's own token | Applied objects, `pruned: …` lines, one sentence per cluster problem | Render anything, or expect `job.spec.suspend` to hold ([Kueue owns it](queueing.md#pause)) |
...
| **warm-up Job** | Its recipe's directory of the model cache PVC (`<pipeline>-<recipe sha256>`, mounted at `/data`), read-write by this pod alone, and the marker `/data/warmup/<pipeline>.done` in it | The pipeline ConfigMap, the cache PVC | The marker file, and a `{stage: warmup}` termination message on failure | Carry the queue label (so Kueue does not gate it), mount S3, write another recipe's directory, or transcribe |
...
| **read API**: `packages/web` | Each campaign's status ConfigMap (`campaign-<name>-status`), by server-side apply | get/list on Jobs and Pods, get/list/create/patch on ConfigMaps (the status ConfigMap's server-side apply) in one namespace — a Role, not a ClusterRole — and anonymous GETs of `progress.json` | `GET /api/v1/jobs[/…]`: phase, counts, per-volume progress, warm-up status, `resultsBase` | Write any other object (a Kyverno rule holds its writes to status ConfigMaps when policies are on), write the bucket, hold S3 credentials, or keep state beyond a few seconds' cache |
...
| **Kyverno** | Admission verdicts in the namespace **when enabled**: under `security.policies.enabled`, digest-pinned images from allowed registries, the job shape (a campaign or warm-up Job exactly as the converter renders it) and the RBAC scope (what the read API and the apply identity may write or delete); model revisions under `requireModelRevision`; signatures under `verifyImages.enabled`. All three switches are off by default | Every Job and Pod CREATE/UPDATE, the converter-labelled ConfigMaps, and the two service identities' writes | An `Enforce` rejection naming what is wrong | Mutate a pod spec, queue anything, or know what a campaign is ([Security](security.md#trust-boundary)) |
